...
$current_filter = $_GET['day'] ?? 'today';

if (!in_array($current_filter, ['today', 'tomorrow', 'yesterday'])) {
    $current_filter = 'today';
}

// start to switch condition
...
$target_date = date('Y-m-d', strtotime("$offset day"));

// $current_date = date('d-m-Y', strtotime($target_date));

$match_day = $title . " ($target_date)";
    ...
    <div class="container">
        <div class="top-bar">
            ...
            <div class="header-titles">
                <h1 class="main-title"><?= htmlspecialchars($match_day) ?></h1>
            </div>
        </div>
